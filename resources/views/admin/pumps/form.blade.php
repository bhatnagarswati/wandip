...
<div class="form-group">
    <label for="store_id">{{ __('admin/pumps.pumpStoreTitle') }} </label>
    <select name="storeId" id="storeId" class="form-control select2">
        <option value=""></option>
        @foreach($stores as $store)
        <option @if(@$pump->storeId == $store->id) selected="selected" @endif
...
<div class="form-group {{ $errors->has('pumpTitle') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="pumpTitle" type="text" id="pumpTitle"
        value="{{ isset($pump->pumpTitle) ? $pump->pumpTitle : ''}}">
    {!! $errors->first('pumpTitle', '<p class="help-block">:message</p>') !!}
</div>
...
<div class="form-group {{ $errors->has('pumpDescription') ? 'has-error' : ''}}">
    ...
    <textarea class="form-control" name="pumpDescription" type="text"
        id="pumpDescription">{{ isset($pump->pumpDescription) ? $pump->pumpDescription : ''}}</textarea>


    {!! $errors->first('pumpDescription', '<p class="help-block">:message</p>') !!}
</div>
<div class="form-group {{ $errors->has('pumpAddress') ? 'has-error' : ''}}">
    ...
    <input name="locationLong" type="hidden" id="locationLong"
        value="{{ isset($pump->pumpLong) ? $pump->pumpLong : ''}}">

    {!! $errors->first('pumpLocation', '<p class="help-block">:message</p>') !!}
</div>
<div class="form-group {{ $errors->has('pumpPrice') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="pumpPrice" type="text"
        value="{{ isset($pump->pumpPrice) ? $pump->pumpPrice : ''}}" id="pumpPrice"></textarea>
    {!! $errors->first('pumpPrice', '<p class="help-block">:message</p>') !!}
</div>
...
<div class="form-group {{ $errors->has('pumpPic') ? 'has-error' : ''}}">
    ...
    <label class="btn btn-default btn-file">
        Browse <input class="form-control showPic" name="pumpPic" id="pumpPic" type="file" style="display: none;"
    ...
    {!! $errors->first('pumpPic', '<p class="help-block">:message</p>') !!}
</div>
...
<div class="form-group {{ $errors->has('pumpMassUnit') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="pumpMassUnit" id="pumpMassUnit">
        ...
        <option value="{{$value}} " @if(!empty($pump)) @if($pump->pumpMassUnit == $value) {{ "selected"}} @endif @endif
            > {{$unit}} {{ "(". $value .")" }}</option>
        @endforeach
...
<div class="form-group {{ $errors->has('status') ? 'has-error' : ''}}">
    ...
    <select class="form-control" name="status" id="status">
        <option value="1" @if(!empty($pump)) @if($pump->status == 1) {{ "selected"}} @endif @endif
            >{{ __('admin/pumps.pump_active') }}</option>
        <option value="0" @if(!empty($pump)) @if($pump->status == 0) {{ "selected"}} @endif @endif
            >{{ __('admin/pumps.pump_inactive') }}</option>
...
<div class="form-group">
    <input class="btn btn-primary" type="submit"
        value="{{ $formMode === 'edit' ? __('admin/common.action_update') :  __('admin/common.action_create') }}">
    <a href="{{ url('/admin/pumps') }}" class="btn btn-warning" title="Back"> <i class="fa fa-arrow-left"
            aria-hidden="true"></i> {{ __('admin/common.action_back') }} </a>
</div>
