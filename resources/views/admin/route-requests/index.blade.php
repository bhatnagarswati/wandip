         ...
         <table class="table table-bordered table-striped">
            ...
            <tbody>
               ...
               <tr>
                  ...
                  <td>
                     <a href="{{ url('/admin/requests/' . $item->id) }}" title="View Route Request"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> {{ __('admin/common.action_view') }}</button></a>			
                     

                     @if($item->status == 1 && $item->markedStatus == 0)	

                     <a href="{{ url('/admin/requests/' . $item->id . '/cancel-request') }}"  onclick="return confirm('Confirm Cancellation? ');" title="Cancel Route Request"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> {{ __('admin/routers.requests_cancel_btn') }}</button></a>
                     @elseif ($item->status  == 0  && $item->markedStatus == 0)	
                     
                     <a href="{{ url('/admin/requests/' . $item->id . '/activate-request') }}"  title="Activate Route Request"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> {{ __('admin/routers.requests_active_btn') }}</button></a>
                    ...
                    <button class="btn btn-primary btn-sm label-success">  {{ __('admin/routers.requests_delivered_btn') }}</button>
                     ...
                     <!-- <a href="{{ url('/admin/requests/' . $item->id . '/edit') }}" title="Edit Route Request"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> {{ __('admin/common.action_edit') }}</button></a>
                     
                     
                     <form method="POST" action="{{ url('/admin/requests' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">				{{ method_field('DELETE') }}				{{ csrf_field() }}				<button type="submit" class="btn btn-danger btn-sm" title="Delete Route Request" onclick="return confirm('Confirm delete? ');"><i class="fa fa-trash-o" aria-hidden="true"></i> {{ __('admin/common.action_delete') }}</button>			    </form> -->
                  </td>
               </tr>
               @endforeach		
            </tbody>
