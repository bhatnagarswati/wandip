<div class="form-group {{ $errors->has('servicerId') ? 'has-error' : ''}}">
    <label for="servicerId" class="control-label">{{ __('admin/stores.service_provider') }}</label>
    <select name="servicerId" id="servicerId" class="form-control select2">
            @foreach($servicers as $key => $servicer)
                <option @if(@$store->servicerId  == $servicer->id) selected="selected" @endif value="{{ $servicer->id }}">{{ $servicer->name }} </option>
            @endforeach
    </select>  
...
<div class="form-group">
   ...
   <a href="{{ url('/admin/stores') }}" class="btn btn-warning" title="Back"> <i class="fa fa-arrow-left" aria-hidden="true"></i> Back </a>
