...
<section class="content">
    <div class=" box">
        <div class="box-body ">
            ...
            <!--  <a href="{{ url('/servicer/stores/create') }}" class="btn btn-success btn-sm" title="Add New Servicer"> <i class="fa fa-plus" aria-hidden="true"></i> {{ __('admin/common.action_add') }} </a>
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody> @foreach($stores as $item)
                    <tr>
                        ...
                        <td>
                            <a href="{{ url('/servicer/stores/' . $item->id) }}" title="View Store">
                                <button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> {{ __('admin/common.action_view') }}</button>
                            </a>
                            <a href="{{ url('/servicer/stores/' . $item->id . '/edit') }}" title="Edit Store">
                                <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> {{ __('admin/common.action_edit') }}</button>
                            </a>
                            <form method="POST" action="{{ url('/servicer/stores' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline"> {{ method_field('DELETE') }} {{ csrf_field() }}
                                <button type="submit" class="btn btn-danger btn-sm" title="Delete Store" onclick="return confirm( & quot; Confirm delete? & quot; )"><i class="fa fa-trash-o" aria-hidden="true"></i> {{ __('admin/common.action_delete') }}</button>
                            </form>
                        </td>
                    </tr> @endforeach </tbody>
            </table>
