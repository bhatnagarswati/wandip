    ...
    <!-- Default box -->
        @if($addresses)
            <div class="box">
                <div class="box-body">
                    <h2>Addresses</h2>
                    @include('layouts.search', ['route' => route('servicer.addresses.index')])
                    <table class="table table-bordered table-striped">
                        ...
                        @foreach ($addresses as $address)
                            <tr>
                                <td><a href="{{ route('servicer.customers.show', [$address->customer_id]) }}">{{ $address->alias }}</a></td>
                                ...
                                <td>
                                    <form action="{{ route('servicer.addresses.destroy', $address->id) }}" method="post" class="form-horizontal">
                                        ...
                                        <div class="btn-group">
                                            <a href="{{ route('servicer.addresses.edit', $address->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> Edit</a>
