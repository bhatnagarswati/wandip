    ...
    <!-- Default box -->
        @if($orders)
            <div class="box">
                <div class="box-body">
                    <h2>Orders</h2>
                    @include('layouts.search', ['route' => route('servicer.orders.index')])
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <td class="col-md-3">Ordered Date</td>
                        ...
                        @foreach ($orders as $order)
                            <tr>
                                <td><a title="Show order" href="{{ route('servicer.orders.show', $order->id) }}">{{ date('M d, Y h:i a', strtotime($order->created_at)) }}</a></td>
