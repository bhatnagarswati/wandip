    ...
    <div class="our_project_bx">
        ...
        <div class="item_content">
            ...
            <h4>{{ config('cart.currency') }}
                ...
                {{ number_format($product->attributes->where('default', 1)->first()->sale_price, 2) }} / {{ ucfirst($product->mass_unit) }}
                <p class="text text-danger">{{ __('common.listproduct_sale') }}!</p>
                @else
                {{ number_format($product->attributes->where('default', 1)->first()->price, 2) }} / {{ ucfirst($product->mass_unit) }}
                @endif
                @else
                {{ number_format($product->price, 2) }} / {{ ucfirst($product->mass_unit) }}
                @endif</h4>
            <a  class="btn blue-btn creative_btn" href="{{ route('front.get.product', str_slug($product->slug)) }}">  <i class="fa fa-cart-plus"></i>  {{ __('common.listproduct_addtocart') }} </a>
           <!--  <form action="{{ route('cart.store') }}" class="form-inline" method="post">
                {{ csrf_field() }}
                <input type="hidden" name="quantity" value="1" />
                <input type="hidden" name="product" value="{{ $product->id }}">
                <button id="add-to-cart-btn" type="submit" class="btn blue-btn creative_btn" data-toggle="modal"
                    data-target="#cart-modal"> <i class="fa fa-cart-plus"></i> Add to cart</button>
            </form> -->
        </div>
    </div>
</div>
