    ...
    <div class="container">
        <div class="dashboard_main_row">
            ...
            <div class="dashboard_content_main">
                ...
                <div class="comn_height dashboard_colm_rt white_bg update_add">
                    <div class="col_sm_12 col_xl_10 ">
                        <div class="manage_adress_main">
                            <h3>{{ __('common.ac_page_update_title')  }}</h3>

                            <form action="{{ route('customer.address.update', [$customer->id, $address->id]) }}"
                                ...
                                <div class="box-body">
                                    ...
                                    <div class="form-group">
                                        <label for="alias">{{ __('common.ac_page_addnew_alias')  }} <span class="text-danger">*</span></label>
                                        <input type="text" name="alias" id="alias" placeholder="{{ __('common.ac_page_addnew_alias')  }}"
                                            class="form-control" value="{{ old('alias') ?? $address->alias }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="address_1">{{ __('common.ac_page_addnew_address1')  }}<span class="text-danger">*</span></label>
                                        <input type="text" name="address_1" id="address_1" placeholder="{{ __('common.ac_page_addnew_address1')  }}"
                                            class="form-control" value="{{ old('address_1') ?? $address->address_1 }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="address_2">{{ __('common.ac_page_addnew_address2')  }}</label>
                                        <input type="text" name="address_2" id="address_2" placeholder=" {{ __('common.ac_page_addnew_address2')  }}"
                                            class="form-control" value="{{ old('address_2') ?? $address->address_2 }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="country_id">{{ __('common.ac_page_addnew_country')  }} </label>
                                        <select name="country_id" id="country_id" class="form-control select2">
                                            @foreach($countries as $country)
                                            <option @if($address->country_id == $country->id) selected="selected" @endif
                                    ...
                                    <div class="form-group">
                                        <label for="zip">{{ __('common.ac_page_addnew_zipcode')  }} </label>
                                        <input type="text" name="zip" id="zip" placeholder="Zip code"
                                            class="form-control" value="{{ old('zip') ?? $address->zip }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="phone">{{ __('common.ac_page_addnew_phoneno')  }}</label>
                                        <input type="text" name="phone" id="phone" placeholder="Phone number"
                                            class="form-control" value="{{ old('phone') ?? $address->phone }}">
                                    </div>
                                </div>
                                <!-- /.box-body -->
                                <div class="box-footer">
                                    <div class="btn-group">
                                    <button type="submit" class="btn blue-btn creative_btn">{{ __('common.ac_page_update_btn')  }}</button>

                                        <a href="{{ url('accounts/addresses') }}"
                                            class="btn btn-default">{{ __('common.ac_page_back_btn')  }}</a>
