...
namespace App\v1\Carts\Repositories;

use App\v1\Stores\Store;
use App\v1\Carts\Exceptions\ProductInCartNotFoundException;
...
class CartRepository extends BaseRepository implements CartRepositoryInterface
{
    ...
    public function addToCart(Product $product, int $int, $options = []): CartItem
    {
        ...
        $cart = $this->model->add($product, $int, $options);
        $productRepo = new ProductRepository(new Product());
        $product = $productRepo->findProductById($cart->id)->toArray();
        $cart->product = $product;
        $this->model->create_cart($cart);
        return $cart;
    }
