...
namespace App\Http\Controllers\Api\v1\Driver\RouteRequests;
...
class RouteRequestController extends Controller
{
    ...
    public function getAllRequestsOnRoute(Request $request)
    {
        ...
        if ($routeType == 0) {
            ...
            // Pending Requests
            $data = RouteRequest::with(['driverinfo', 'servicerinfo', 'customerinfo'])->where(['driverId' => $this->userId, 'routeId' => $routeId, 'status' => 1, 'markedStatus' => 0]);
        ...
        if ($routeType == 1) {
            ...
            // Delivered Requests
            $delivered_data = RouteRequest::with(['driverinfo', 'servicerinfo', 'customerinfo'])->where(['driverId' => $this->userId, 'routeId' => $routeId, 'status' => 1, 'markedStatus' => 1]);
            if (!empty($request->input('search_key'))) {
                $keyword = $request->input('search_key');
    ...
    public function markDelivered(Request $request)
    {
        ...
        $request_updated = RouteRequest::where(['id' => $requestId, 'driverId' => $this->userId])->firstOrFail();
        if ($request_updated) {
