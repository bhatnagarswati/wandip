...
namespace App\Http\Controllers\Api\v1\Customer\Orders;
...
class OrderController extends Controller
{
    ...
    public function customerOrders(Request $request)
    {
        ...
        $data = Order::with('products', 'customer','servicer', 'address')->where(['customer_id' => $this->userId]);
            ...
            foreach ($orders as $key => $order) {
                ...
                if (!empty($order->products)) {
                    ...
                    foreach ($order->products as $product) {
                        ...
                        // Add Rating
                        $product->rating = 0;
                        $reviews = ProductReviews::where(['customerId' => $this->userId , 'productId' => $product->id])->first();
                        if(!is_null($reviews)){
                            $product->rating = $reviews->customerRating;
                        }
        ...
        $this->success("Customer orders list", $response);
    }
