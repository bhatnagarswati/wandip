...
class OrderController extends Controller
{
    ...
    {
        $list = $this->orderRepo->listOrders('created_at', 'desc')->where('servicerId', Auth::guard('servicer')->user()->id);

        if (request()->has('q')) {
            $list = $this->orderRepo->searchOrder(request()->input('q') ?? '')->where('servicerId', Auth::guard('servicer')->user()->id);
        }
        $orders = $this->orderRepo->paginateArrayResults($this->transFormOrder($list), 15);
        return view('servicer.orders.list', ['orders' => $orders]);
    }
